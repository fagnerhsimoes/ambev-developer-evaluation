...
public class CreateSaleHandler : IRequestHandler<CreateSaleCommand, CreateSaleResult>
{
    ...
    public async Task<CreateSaleResult> Handle(CreateSaleCommand command, CancellationToken cancellationToken)
    {
        ...
        if (customer == null)
            throw new ValidationException(new[] { new FluentValidation.Results.ValidationFailure("CustomerId", $"Customer with ID {command.CustomerId} not found") });
        ...
        // Initialize Status
        sale.Status = SaleStatus.Pending; // Or Completed, depending on flow. Using Pending for now.
        ...
        var eventCreated = new SaleCreatedEvent(createdSale);
